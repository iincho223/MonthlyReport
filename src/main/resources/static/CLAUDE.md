# フロントエンドルール（src/main/resources/static/**, doc/prototype/**）

このディレクトリ配下の Vue.js / CSS / axios 通信コードを設計・実装する際に適用するルール。プロジェクト全体のルールは [ルート CLAUDE.md](../../../../CLAUDE.md) を参照。
`doc/prototype/**` 配下のプロトタイプ実装にも同じルールを適用する。
...
- ロールは `NG/TM/TL/GL/OM/SP/SM/SA` の8種のみを使用する。旧称（`REPORTER` など）を新規に追加しない。
