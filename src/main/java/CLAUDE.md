# バックエンドルール（src/main/java/**）

このディレクトリ配下の Java コードを設計・実装する際に適用するルール。プロジェクト全体のルールは [ルート CLAUDE.md](../../../../CLAUDE.md) を参照。
...
- 既存の命名・パッケージ規約を崩さない。
- ロールは `UserRole`（`NG/TM/TL/GL/OM/SP/SM/SA`）の8種のみを使用する。旧称（`REPORTER` など）を新規に追加しない。
